...
- [ ] Close the zlib row-flush state-model gap: continuous-prefix replay sees both target and zlib choices, while row-chunked replay sees target but not zlib's actual choice, so we need either an instrumented zlib oracle or a closer `deflate_slow` flush-boundary state model.
